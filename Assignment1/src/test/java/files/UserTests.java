...
public class UserTests extends Data {
	...
	private void test_get_single_user_returns_http_200_UStory1(int id, String email, String first_name, String last_name, String avatar, String company, String url, String text) {
		RestAssured.baseURI = "https://reqres.in/";
	
		GetSingleUser gsUser =given().queryParam("page",id).expect().defaultParser(Parser.JSON)
				.when().get("/api/users/"+id)
				.then().spec(res200)
				.extract().response().as(GetSingleUser.class);
	...
	public void test_create_user_returns_http_201_UStory7()
		{	
			...
			.body(au)
			.when().post("/api/users")
			.then().assertThat().statusCode(201).extract().body();
		...
		public void test_register_unsuccessful_user_returns_http_400_UStory12() {
			...
			ResponseBodyExtractionOptions registerUnsuccessRes =given().spec(req)
			.body(ru)
			.when().post("/api/register")
		...
		public void test_login_successful_user_returns_http_200_UStory13(String email, String password) {
			...
			ResponseBody loginSuccessRes = given().spec(req)
			.body(ls)
			.when().post("/api/login")
		...
		public Object[][] getUserNamePg2() {
			return new Object[][] {{0,7,"Michael"},{1,8, "Lindsay"} ,{2,9,"Tobias"}, {3,10, "Byron"}, {4,11, "George"}, {5, 12, "Rachel"} };
		}
		...
		public Object[][] getUserdata() {
			...
			return new Object[][] 
				{
					{1,"george.bluth@reqres.in","George","Bluth","https://s3.amazonaws.com/uifaces/faces/twitter/calebogden/128.jpg","StatusCode Weekly", "http://statuscode.org/","A weekly newsletter focusing on software development, infrastructure, the server, performance, and the stack end of things."},
					{2,"janet.weaver@reqres.in","Janet","Weaver","https://s3.amazonaws.com/uifaces/faces/twitter/josephstein/128.jpg","StatusCode Weekly","http://statuscode.org/","A weekly newsletter focusing on software development, infrastructure, the server, performance, and the stack end of things."},
					{3,"emma.wong@reqres.in","Emma","Wong","https://s3.amazonaws.com/uifaces/faces/twitter/olegpogodaev/128.jpg", "StatusCode Weekly","http://statuscode.org/","A weekly newsletter focusing on software development, infrastructure, the server, performance, and the stack end of things."},
					{12,"rachel.howell@reqres.in","Rachel","Howell","https://s3.amazonaws.com/uifaces/faces/twitter/hebertialmeida/128.jpg", "StatusCode Weekly","http://statuscode.org/","A weekly newsletter focusing on software development, infrastructure, the server, performance, and the stack end of things."}
				};
		}
		...
		{
			return new Object[][] {{"eve.holt@reqres.in","cityslicka"},{"eve.holt@reqres.in","licka"},{"eve.holt@reqres.in","city"}};
